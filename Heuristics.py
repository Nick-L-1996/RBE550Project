...
class heuristicMud(heuristic): #High cost for making turns
    def getHeuristic(self, CurrentNode, NodeToBeExplored, EndNode):
        ...
        cost = np.sqrt(
            np.power(EndNode.xcoord - NodeToBeExplored.xcoord, 2) + np.power(EndNode.ycoord - NodeToBeExplored.ycoord,
                                                                             2))
...
class heuristicTrees(heuristic):
     def getHeuristic(self, CurrentNode, NodeToBeExplored, EndNode):
        if NodeToBeExplored.Environment == "Trees":
            ...
            cost = np.sqrt(np.power(EndNode.xcoord - NodeToBeExplored.xcoord, 2) + np.power(
                EndNode.ycoord - NodeToBeExplored.ycoord, 2))
        return cost
